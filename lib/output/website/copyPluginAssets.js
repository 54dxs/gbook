...
/**
    从插件复制他对应的所有assets资产文件.
    asset资产文件是存储在"_assets"中的文件
    在插件自身声明的nd资源
...
function copyAssets(output, plugin) {
    ...
    var assetOutputFolder = path.join(outputRoot, 'gbook');
...
function copyResources(output, plugin) {
    ...
    var assetOutputFolder = path.join(outputRoot, 'gbook', plugin.getNpmID());
