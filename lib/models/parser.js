...
/**
 * 检查此分析器是否与扩展名匹配
 * @param {String} ext 要校验的扩展名
 * @return {Boolean} true:可解析该扩展名;false:不可解析
 */
...
/**
 * 使用模块 (gbook-markdown, gbook-asciidoc) 创建新的解析器
 * @param {String} name 解析器名称
 * @param {Array<String>} extensions 扩展名(数组 例如:['.md', '.markdown', '.mdown'])
 * 或['.adoc', '.asciidoc']
 * @param {Object} module 解析器模型(npm插件)
 * @return {Parser}
 */
